...
#calculate lowest common multiple of all divisors so we
#can keep the item values low
divisorLCM=[]
for i in range(len(monkeys)):
    divisorLCM.append(monkeysInfo[i][3])
divisorLCM=math.lcm(*divisorLCM)
...
for r in range(10000):
    ...
    for monkeyIndex in range(len(monkeys)):
        for itemIndex in range(len(monkeys[monkeyIndex])):
            ...
            #mod the item with the lowest common multiple of all 
            #divisors, to keep the numbers low!
            if(item>=divisorLCM): item=item%divisorLCM
